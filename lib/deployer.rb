...
module Deployer
  class Deployment
    def initialize(deploy_destination, repository, version, name, logger)
    ...
    def prepare_working_copy (working_copy)
      ...
      if File.directory? working_copy
        @@logger.info { "A direcotry with the same name as the working copy, #{working_copy}, already exists moving directory to #{working_copy}-#{time_stamp}.bak" }
